...
ONE_DRIVER_CONF = "#{ETC_LOCATION}/opennebula_driver.conf"
ONE_DRIVER_DEFAULT = "#{ETC_LOCATION}/opennebula_driver.default"
...
class OpenNebulaDriver
  ...
  def initialize(host, host_id=nil)
    ...
    driver_conf = YAML::load(File.read(ONE_DRIVER_CONF))

    regions = driver_conf['regions']
    region  = regions[host]
    ...
    if region.nil?
      raise "Region not found for host #{host}, and default not found in #{ONE_DRIVER_CONF}"
    end
    ...
    ["user", "password", "endpoint", "capacity"].each do |key|
      ...
      if value.nil? || value == ""
        if found
          raise "Region for host #{host} does not have '#{key}' defined in #{ONE_DRIVER_CONF}"
        else
          raise "Region for host #{host} not found, and default region does not have '#{key}' defined in #{ONE_DRIVER_CONF}"
        end
      end
